...
C2DM.prototype.send = function(packet, cb) {
    ...
    operation.attempt(function(currentAttempt) {
        ...
        var request = https.request(self.c2dmOptions, function(res) {
            ...
            console.log('https request res statusCode :' + res.statusCode);
            ...
            res.on('data', function(chunk) {
                data += chunk;
                console.log('https req res data:', data);
            });
            res.on('end', respond);
            res.on('close', respond);
        });
...
C2DM.prototype.sendWithApiKey = function(msgObj, cb) {
    var self = this;
    if (cb) this.once('sent', cb);
    ...
    operation.attempt(function(currentAttempt) {
        ...
        console.log('http options:', self.c2dmOptions);
        var request = https.request(self.c2dmOptions, function(res) {
            var data = '';
        ...
        // now post the msg json object.
        request.write(JSON.stringify(msgObj));
        request.end();
    });
};
